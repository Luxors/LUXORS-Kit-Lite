	...
	return new Promise((resolve, reject) => {
		...
		if (isValid) {
			...
			const errMsg = (
				...
				`ERR>>> A block name must include letters, numbers & the minus symbol.`.red
			);
			reject(errMsg);
		}
	});
...
function getFiles(blockPath) {
	return new Promise((resolve, reject) => {
		fs.readdir(blockPath, (err, files) => {
			if (err) {
				reject(`ERR>>> Failed to get a file list from a folder '${blockPath}'`.red);
			} else {
				resolve(files);
			}
		});
...
function initMakeBlock(blockName) {
	...
	return validateBlockName(blockName)
			...
			.then(files => {
				...
				console.log(line);
				console.log(`The block has just been created in 'app/blocks/${blockName}'`.green);
				console.log(line);
